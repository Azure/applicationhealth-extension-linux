...
func Test_reportStatus_fails(t *testing.T) {
	...
	akeEnv.StatusFolder = "/non-existing/dir/"  // intentionally set to a non-existing directory
...
func Test_reportStatus_fileExists(t *testing.T) {
	...
	fakeLogger := logging.NewExtensionLogger(fakeEnv)
	require.NoError(t, err, "failed to create logger")
	// slog.SetDefault(fakeLogger)

	require.Nil(t, ReportStatus(fakeLogger, fakeEnv, 1, status.StatusSuccess, nil, ""))
...
func Test_reportStatus_checksIfShouldBeReported(t *testing.T) {
	...
	for _, c := range cmds.Values() {
		...
		_, err = telemetry.NewTelemetry(fakeEnv)
		require.NoError(t, err, "failed to initialize telemetry")
		fakeLogger, err := logging.NewExtensionLogger(fakeEnv)
		require.NoError(t, err, "failed to create logger")

		require.Nil(t, ReportStatus(fakeLogger, fakeEnv, 2, status.StatusSuccess, c, ""))
