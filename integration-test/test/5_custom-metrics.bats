...
@test "handler command: enable - custom metrics - sending empty string custom metrics is omitted and not seen in status file " {
    mk_container sh -c "webserver -args=2h-empty,2h-empty,2u-empty,2u-empty & fake-waagent install && fake-waagent enable && wait-for-enable webserverexit"
    push_settings '
    {
        "protocol": "http",
        "requestPath": "health",
...
@test "handler command: enable - custom metrics - sending invalid formatted custom metrics result in unknown" {
    mk_container sh -c "webserver -args=2h-invalid,2h-invalid,2u-invalid,2u-invalid & fake-waagent install && fake-waagent enable && wait-for-enable webserverexit"
    push_settings '
    {
        "protocol": "http",
        "requestPath": "health",
...
@test "handler command: enable - custom metrics - sending valid custom metrics is seen in status file" {
    mk_container sh -c "webserver -args=2h-valid,2h-valid,2u-valid,2u-valid & fake-waagent install && fake-waagent enable && wait-for-enable webserverexit"
    push_settings '
    {
        "protocol": "http",
        "requestPath": "health",
